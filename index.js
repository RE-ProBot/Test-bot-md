    ...
    try {
 	...
 	var budy = (typeof m.text == 'string' ? m.text : '')
       var prefix = prefa ? /^[°•π÷×¶∆£¢€¥®™+✓_=|~!?@#$%^&.©^]/gi.test(body) ? body.match(/^[°•π÷×¶∆£¢€¥®™+✓_=|~!?@#$%^&.©^]/gi)[0] : "" : prefa ?? global.prefix
        ...
        const wita = moment(Date.now()).tz('Europe/Berlin').locale('id').format('HH:mm:ss z')
        const wit = moment(Date.now()).tz('Europe/Berlin').locale('id').format('HH:mm:ss z')
		...
		const fgclink = {key: {participant: "0@s.whatsapp.net","remoteJid": "0@s.whatsapp.net"},"message": {"groupInviteMessage": {"groupJid": "777777777777-1616169743@g.us","inviteCode": "m","groupName": "P", "caption": `© ${ownername}`, 'jpegThumbnail': pp_bot}}}
		const fvideo = {key: { fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "777777777777-1613049930@g.us" } : {}) },message: { "videoMessage": { "title":`${pushname}`, "h": `Hmm`,'seconds': '359996400', 'caption': `${pushname}`, 'jpegThumbnail': pp_bot}}}
		const floc = {key : {participant : '0@s.whatsapp.net'},message: {locationMessage: {name: `${ownername}`,jpegThumbnail: pp_bot}}}
		const fkontak = { key: {participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: `777777777777-1614953337@g.us` } : {}) }, message: { 'contactMessage': { 'displayName': `${pushname}`, 'vcard': `BEGIN:VCARD\nVERSION:3.1\nN:XL;${pushname},;;;\nFN:${pushname},\nitem1.TEL;waid=${sender.split('@')[0]}:${sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`, 'jpegThumbnail': pp_bot, thumbnail: pp_bot,sendEphemeral: true}}}
...
Doing : ${clockString(new Date - afkTime)}
`.trim())
}
...
*Create Time : ${moment(value.at * 1).tz('Europe/Berlin').format('DD/MM/YYYY HH:mm:ss')}*
*Locked : ${value.locked}*
`).join('\n')}
`.trim()
...
if (!m.isGroup) return reply(lang.groupOnly())
        ...
        reply(lang.OffBef('Nsfw'))
			...
			case 'cekproses':{
				if (!m.isGroup) return reply(lang.groupOnly())
				defaultwel = `「 *PENDING TRANSACTION* 」\n\n\`\`\`📆 DATE : @DATE\n⌚ JAM     : @jam\n✨ STATUS  : Pending\`\`\`\n\n📝 Catatan :\n@catatan\n\nPesanan @user sedang di proses!`
...
teks += `🐥 *Name :* ${metadata.subject}\n👤 *Owner :* @${groupMetadata.owner.split('@')[0]}\n♨️ *ID :* ${metadata.id}\n🗓️ *Made :* ${moment(metadata.creation * 1000).tz('Europe/Berlin').format('DD/MM/YYYY HH:mm:ss')}\n👥 *Member :* ${metadata.participants.length}\n\n────────────────────────\n\n`
}
alpha.sendTextWithMentions(m.chat, teks, m)
}
break
               case 'listgroup': case 'listgc': {
                 ...
                 for (let i of anu) {
                     let metadata = await alpha.groupMetadata(i)
                     teks += `⬡ *Name :* ${metadata.subject}\n⬡ *Owner :* ${metadata.owner !== undefined ? '@' + metadata.owner.split`@`[0] : '-'}\n⬡ *ID :* ${metadata.id}\n⬡ *Made :* ${moment(metadata.creation * 1000).tz('Europe/Berlin').format('DD/MM/YYYY HH:mm:ss')}\n⬡ *Member :* ${metadata.participants.length}\n\n────────────────────────\n\n`
                 }
                 alpha.sendTextWithMentions(m.chat, teks, m)
            }
                 break
...
console.log("Antwort: " + result.jawaban)
  ...
  {buttonId: 'donasi', buttonText: {displayText: '💰 Donation'}, type: 1}
					...
					if(typemenu == 'document'){
						if(db.data.users[m.sender].registered){
							alpha.sendButDoC(m.chat, ini_anu,  '© ' + ownername, botname , ownername, `Neo Enterprise`, time, pp_bot, pp_bot, buttojns, [ownernya, ini_mark, m.sender], { quoted: ftroli})
						} else alpha.sendButDoc(m.chat, ini_anu,  '© ' + ownername, botname , ownername, `Neo Enterprise`, time, pp_bot, pp_bot, buttojns, [ownernya, ini_mark, m.sender], { quoted: ftroli})
					}
	...
	case 'infocmd': case'infomenu':{
		alpha.sendButImage(m.chat, sender, [{buttonId: '.command',buttonText:{displayText: '༺ Back'},type: 1},{buttonId:'owner',buttonText: {displayText:'Owner ༻'}, type: 1}], `Hi ${salam} @${sender.split('@')[0]} 😊\n\n╭─⬣「 _*INFO BOT*_ 」⬣\n│ *Prefix* :  ${prefix} \n│ *Name* : ${botname}\n│ *Owner* : @${ownernomer.split("@")[0]}\n│ *Mode* : ${alpha.public ? 'Public-Mode' : 'Self-Mode'}\n│ *Runtime* : ${runtime(process.uptime())}\n│ *Lib* : Baileys-Md@4.0.0\n╰─⬣` + '\n\n' +  lang.info(prefix), `© ${ownername}`, [sender, ownernomer + '@s.whatsapp.net'], {quoted:m})
      }
  break 
...
if (!text) return reply(lang.KisahNabi(prefix, command, 'Makassar'))
	...
	.then(async data =>{
		...
		txt += `• Daerah : ${q}\n\n`
		txt += `• Date : ${moment(new Date()).tz('Europe/Berlin').locale('id').format('HH:mm:ss DD/MM/YYYY')}\n`
